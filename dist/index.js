...
async function getMostRecentReleaseCandidateNumber(supabaseClient, tableName, {
  ...
  if (!isUndefined_default()(name)) query = query.eq('name', name)
  if (!isUndefined_default()(version)) query = query.eq('version', version)
  if (!isUndefined_default()(region)) query = query.eq('region', region)
  if (!isUndefined_default()(namespace)) query = query.eq('namespace', namespace)
